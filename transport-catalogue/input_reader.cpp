...
void InputReader::ApplyCommands([[maybe_unused]] TransportCatalogue& catalogue) const {
    ...
    for (const auto& cmd : bus_commands) {
        std::vector<std::string_view> route_stops = ParseRoute(cmd.description);

        bool is_circle = (route_stops.front() == route_stops.back());

        domain::Bus bus{std::string(cmd.id), is_circle, {}};

        for (const auto& stop_name : route_stops) {
